...
int main()
{
	...
	Player p1("hero.png", 96, 96, 54, 54);	// Создание и первоначальное размещение персонажа
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (event.type == Event::MouseButtonPressed)	// Если была нажата кнопка мыши
			{
				...
				if (event.key.code == Mouse::Right && p1.m_isSelect)
				{
					...
					float d_x = pos.x - p1.getX();	// вектор, колинеарной прямой, которая пересекает спрайт и курсор
					float d_y = pos.y - p1.getY();	// тоже самое по оси y
		...
		if (p1.m_isMove)
		{
			...
			distance = sqrt(((tempX - p1.getX()) * (tempX - p1.getX())) + ((tempY - p1.getY()) * (tempY - p1.getY())));
			if (distance > 5)	// Этим условием убираем дерганье спрайта в конейной точке движения
			{
				p1.setX(p1.getX() + (0.2f * time * (tempX - p1.getX()) / distance)); // Движение по х, с помощбю вектора нормали
				p1.setY(p1.getY() + (0.2f * time * (tempY - p1.getY()) / distance));	// Движение по y, с помощбю вектора нормали
			}
			else
			{
				p1.m_isMove = false;
		...
		text_score.setPosition(p1.getX() - p1.m_sprite.getOrigin().y * 2, p1.getY() - p1.m_sprite.getOrigin().y);	// Прикрепляем вывод текста к персонажу
